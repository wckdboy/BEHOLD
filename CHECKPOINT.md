...
| | |
| --- | --- |
| **Version on `main` (before this cut)** | 0.16.0 |
| **This branch / after merge** | **0.17.0** |
...
| **Install zip** | `dist/behold-0.17.0.zip` (Actions artifact `behold-addon`) |
...
6. **Shoot** — **Draft / Final**, **Still**, **Render**, compact **Look** (EV / Kelvin WB / False Color), compact **Shots** (list / Add / Apply / rename / Delete), compact **Turntable** (seconds + Setup + Play). Last first-class section. No batch, path tokens, camera bookmark, bake, or ease on this panel.
...
- Bake-to-HDRI.
- Shot Manager changes (EV / WB / False Color are scene Look, not stored on a shot).
...
- **Galahad** owns backends and the tessellator (import, OCP/STEPper, studio build, HDRI world nodes, lights + shape presets, cameras, shoot operators, exposure apply, shot serialize/apply, turntable rig, STEP vertical smoke, STEPper NEXT first-class import, local material apply, GitHub update check / zip install).
- **Percival** owns panel chrome (what the N-panel shows, what stays collapsed in Advanced, official mark placement, compact Studio HDRI card, compact Shoot Shots card, compact Shoot Look card, Lights shape row).
