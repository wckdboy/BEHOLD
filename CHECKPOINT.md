...
| | |
| --- | --- |
| **Version on `main` (before this cut)** | 0.7.0 |
| **This branch / after merge** | **0.8.0** |
...
| **Install zip** | `dist/behold-0.8.0.zip` (Actions artifact `behold-addon`) |
...
N-panel keeps the three-click path skinny. Lights, Cameras, and Materials are dedicated sections after Shoot. Turntable is a compact row on Shoot — not a fourth first-class section.
...
7. **Advanced** (`BEHOLD_PT_advanced`, `DEFAULT_CLOSED`) — mixer, CAD box, BlenderKit login/search/apply, Light Draw hotkeys, batch, turntable spin / bake / clear / render, Bookmark / Use Main.
